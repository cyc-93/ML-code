...
from xgboost import XGBRegressor # Changed from SVR to XGBRegressor
    ...
    while q >= 2:
        try:
            ...
            if counts.min() >= n_splits:
                return bins
        except ValueError:  # Not enough unique values for bins
            pass
        q -= 1
    return None
...
search_space_xgb = {
    ...
    'reg_alpha': Real(1e-3, 100.0, prior='log-uniform'),  # L1 regularization
    'reg_lambda': Real(1e-3, 100.0, prior='log-uniform'), # L2 regularization
}

outer_splits = 10
all_params = []

print(f"[INFO] Using scikit-learn XGBRegressor")
with pd.ExcelWriter('final_tuned_metrics_xgb_final20.xlsx', engine='openpyxl') as metrics_writer, \
        pd.ExcelWriter('final_tuned_predictions_xgb_final20.xlsx', engine='openpyxl') as preds_writer:
    ...
    for s in sheet_names:
        ...
        for k, (tr_idx, te_idx) in enumerate(split_iter, start=1):
            ...
            base = XGBRegressor(
                ...
                eval_metric='rmse',
                random_state=42,
                n_jobs=1  # Let BayesSearchCV handle parallelization
            )
            ...
            opt = BayesSearchCV(
                estimator=base,
                search_spaces=search_space_xgb, # Using XGBoost search space
                n_iter=50,
                cv=inner_cv,
            ...
            # Record the best parameters for this fold
...
pd.DataFrame(all_params).to_csv('final_best_xgb_params_final20.csv', index=False)
